...
public class MemberDAOlbs {
	...
	public Connection getConn() {
		try {	connection = dataSource.getConnection();
				...
				System.out.println("MemberDAOlbs.getConn() Exception!!!");
		}
		return connection;
	}//getConn()
	...
	public void dbClose() {
		try {	if(resultSet != null)			resultSet.close();
				...
				System.out.println("MemberDAOlbs.dbClose() Exception!!!");		
		}
	}//dbClose()
	...
	//회원-탈퇴 처리
	//tbl_member.탈퇴요청 회원의 member_kind(구분코드)를 { 9 }로 변경하고,
	//member_date_delete(개인정보 삭제 예정일)를 생성,저장한다.
	public int anMemberCancel(int member_code) {
		...
		try {
			System.out.println("member_code: " + member_code + "번 회원님이 탈퇴 요청하였습니다.");
			...
			if (state > 0)	System.out.println("탈퇴회원(9)으로 변경 성공!");				
			else			System.out.println("탈퇴회원(9)으로 변경 실패~");

		} catch(Exception e){ e.printStackTrace();
			System.out.println("MemberDAO.anMemberCancel() Exception!!!");
		} finally{ dbClose(); }
	...
	//회원-개인정보삭제 처리
	//서버에 있는 member_image 파일 삭제. 
	//tbl_member.member_date_delete(개인정보 삭제 예정일)가 경과한 탈퇴회원들의 개인정보를 삭제한다.
	public int anMemberDeleteInfo() {
		...
		//서버에 있는 member_image 파일 삭제 처리(예정)
		//ArrayList<String> list = new ArrayList<>();		
		try { System.out.println("member_date_delete(개인정보 삭제 예정일)이 경과한 회원님들의 사진파일을 삭제합니다.");
			...
			while(resultSet.next()) {
				...
				System.out.println("1. " + member_image);
				
				// 이미지 파일지우기
				File delfile = new File(member_image);
				System.out.println("2. " + delfile.getAbsolutePath());
				
				if(delfile.exists()) {
					System.out.print("3. Del:member_image " + delfile.exists() + " --> ");
					boolean deleteFile = false;
					while(deleteFile != true){
						deleteFile = delfile.delete();
					}//while()     
					System.out.println("삭제완료: " + deleteFile + "\n");
				}//if(delfile.exists())
			}//while(resultSet.next())
			
		} catch (Exception e) { e.printStackTrace();
			System.out.println("MemberDAO.anMemberDeleteInfo().member_image 파일삭제 Exception!!!");
		} finally { dbClose(); }
		...
		try { System.out.println("member_date_delete(개인정보 삭제 예정일)이 경과한 회원님들의 개인정보를 삭제합니다.");
			...
			if (state > 0)	System.out.println("개인정보 삭제 성공!");				
			else			System.out.println("개인정보 삭제 실패~");

		} catch (Exception e) { e.printStackTrace();
			System.out.println("MemberDAO.anMemberDeleteInfo() Exception!!!");
		} finally { dbClose(); }
